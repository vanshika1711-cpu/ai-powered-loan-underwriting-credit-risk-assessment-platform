...
def analytics():
    ...
    approved = conn.execute(
        "SELECT COUNT(*) as count FROM applications WHERE decision='Approved'"
    ).fetchone()["count"]

    rejected = conn.execute(
        "SELECT COUNT(*) as count FROM applications WHERE decision='Rejected'"
    ).fetchone()["count"]

    avg_risk = conn.execute(
        "SELECT AVG(risk) as avg FROM applications"
    ).fetchone()["avg"]
...
def get_audit_logs():
    ...
    rows = conn.execute(
        "SELECT event, details, timestamp FROM audit_logs ORDER BY id DESC"
    ).fetchall()
